...
public static class InputGamepad
{
    ...
    public static void Example()
    {
        ...
        while (!WindowShouldClose())    // Detect window close button or ESC key
        {
            ...
            {
                DrawText(string.Format("GP1: {0}", GetGamepadName(0)), 10, 10, 10, Black);
                ...
                if (TextIsEqual(GetGamepadName(0), XBOX360_NAME_ID) || TextIsEqual(GetGamepadName(0), XBOX360_LEGACY_NAME_ID))
                {
                    ...
                    //DrawText(string.Format("Xbox axis LT: %02.02f", GetGamepadAxisMovement(0, GAMEPAD_AXIS_LEFT_TRIGGER)), 10, 40, 10, Black);
                    //DrawText(string.Format("Xbox axis RT: %02.02f", GetGamepadAxisMovement(0, GAMEPAD_AXIS_RIGHT_TRIGGER)), 10, 60, 10, Black);
                }
                ...
                DrawText(string.Format("DETECTED AXIS [{0}]:", GetGamepadAxisCount(0)), 10, 50, 10, Maroon);

                for (int i = 0; i < GetGamepadAxisCount(0); i++)
                {
                    DrawText(string.Format("AXIS {0}: {1}", i, GetGamepadAxisMovement(0, (GamepadAxis)i).ToString("0.00")), 20, 70 + (20 * i), 10, Darkgray);
                }

                if (GetGamepadButtonPressed() != -1)
                {
                    DrawText(string.Format("DETECTED BUTTON: {0}", GetGamepadButtonPressed()), 10, 430, 10, Red);
                }
                else
                {
